        ...
        <div id="form_inner">
            ...
            if(($han = fopen($file_path,"r")) !== false){
                ...
                for( $i=0 ; $i<count( $data ) ; $i++ ) {
                    $id[$i]=ltrim($id[$i], '0');
                    if(strpos($name[$i],'&cma;') !== false){
                        $name[$i] = str_replace('&cma;',',',$name[$i]);
                    }
                    if(strpos($place[$i],'&cma;') !== false){
                        $place[$i] = str_replace('&cma;',',',$place[$i]);
                    }
                    if(strpos($naiyou[$i],'&cma;') !== false){
                        $naiyou[$i] = str_replace('&cma;',',',$naiyou[$i]);
                    }
                    if(strpos($name[$i],'&lst;') !== false){
                        $name[$i] = str_replace('&lst;','<',$name[$i]);
                        $name[$i] = str_replace('&gst;','>',$name[$i]);
                    }
                    if(strpos($place[$i],'&cma;') !== false){
                        $place[$i] = str_replace('&lst;','<',$place[$i]);
                        $place[$i] = str_replace('&gst;','>',$place[$i]);
                    }
                    if(strpos($naiyou[$i],'&cma;') !== false){
                        $naiyou[$i] = str_replace('&lst;','<',$naiyou[$i]);
                        $naiyou[$i] = str_replace('&gst;','>',$naiyou[$i]);
                    }
                    if($sex[$i]=="male"){
                        $sex[$i]="男性";
                    }elseif($sex[$i]=="female"){
                    ...
                    echo "\t<tr>\n";
                            ...
                            echo "\t\t<td width='120'>{$category[$i]}</td>\n";
